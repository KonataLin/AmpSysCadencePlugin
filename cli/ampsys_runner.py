...
def should_delegate_to_core() -> bool:
...
def write_skill_result(result_json: Path, skill_path: Path, lib: str = "", cell: str = "") -> None:
    ...
    lines = [
        "; Auto-generated by AmpSys Cadence Plugin",
        "ampsys_result_devices = list(",
    ]
    for dev in data.get("devices", []):
        params = [
            ("w", fmt_u(dev.get("W", 0.0))),
            ("W", fmt_u(dev.get("W", 0.0))),
            ("l", fmt_u(dev.get("L", 0.0))),
            ("L", fmt_u(dev.get("L", 0.0))),
            ("fingers", str(dev.get("fingers", 1))),
            ("nf", str(dev.get("fingers", 1))),
            ("nfin", str(dev.get("fingers", 1))),
            ("nFin", str(dev.get("fingers", 1))),
            ("m", "1"),
        ]
        param_expr = " ".join(f'list("{k}" "{v}")' for k, v in params)
        lines.append(f'  list("{dev["name"]}" list({param_expr}))')
    for name, value in (data.get("passives", {}) or {}).items():
        ...
        param_expr = " ".join(f'list("{k}" "{v}")' for k, v in params)
        lines.append(f'  list("{name}" list({param_expr}))')
    lines.append(")")
    if lib and cell:
        lines.append(f'ampsys_result_lib = "{lib}"')
        lines.append(f'ampsys_result_cell = "{cell}"')
...
def run_optimize(project_path: Path) -> None:
    ...
    try:
        ...
        write_skill_result(result_path, skill_path, cad.get("lib", ""), cad.get("cell", ""))
