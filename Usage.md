...
`Spectre threads=auto` 时，AmpSys 会自动估算每个 Spectre 进程的 `+mt`，避免 NMOS/PMOS 两路并行时过度抢占 CPU；如果用户已经在 `Spectre cmd` 中显式写了 `+mt=4` 之类的参数，则以用户填写为准。不同 Spectre 版本的多线程参数可能不同，必要时先用 `spectre -help all` 确认。AmpSys 已经会并行构建 NMOS/PMOS，并将 L/VDS/VSB 合并成大 batch，默认不再额外并发多个 Spectre batch。Linux 服务器大核心数场景可用环境变量 `AMPSYS_SPECTRE_BATCH_WORKERS` 手动打开 batch 并发。
