...
| Spectre cmd | Spectre 命令及可选参数，例如 `spectre` 或按本机版本填写多线程参数 |
| Spectre threads | 默认 `auto`。AmpSys 会按机器 CPU 数、NMOS/PMOS 并行建表策略自动给每个 Spectre 进程设置 `+mt=N` |
| Spectre accel | 默认 `auto`，等价于优先尝试 `+aps`，失败时自动回退普通 Spectre |
...
`AutoSearch Spectre` 只查找 Spectre 可执行文件，不会查找或猜测 PDK/model。`Spectre accel=auto` 时，AmpSys 会按 `+aps` 运行；若当前 Spectre 版本、license 或环境不支持 APS，会自动去掉 APS 参数再跑一次普通 Spectre。LUT 建表只允许 plain `+aps` / `++aps`，不允许 `+aps=liberal`、`+aps=moderate`、`+aps=conservative` 等 preset 模式，避免为了速度放松仿真精度。当前 LUT flow 也不使用 `+xps`，因为 MMSIM151 等版本会要求额外 `+cktpreset=sram/flash`，不适合通用 MOS DC LUT 建表。

部分 Spectre PDK 需要多个 section 或非标准 MOS 参数名。`Corner/lib` 可以用逗号填写多个 section，AmpSys 会按顺序 include，例如 `stat,total_rf_mos,tt_rfmos,ttg_rfmos`。如果 PDK 的器件参数不是 `w/l`，在 LUT 高级设置里修改 `Spectre W param` / `Spectre L param`，例如 TSMC RF inline subckt 可用 `wr` / `lr`，并在 `Spectre extra params` 中填写类似 `nr=1 totalflag=1 globalflag=1 mismatchflag=1` 的附加实例参数。

`Benchmark Spectre` 只跑极小 grid，用来比较当前机器上 `off`、`+aps`、`++aps` 的建表耗时、日志和 cache 文件结构；它不会替代完整 LUT 建表，也不会改变真实 cache。确认 benchmark 正常后，再按需要点击 `Build Library` 建完整 LUT。

`Spectre threads=auto` 时，AmpSys 会自动估算每个 Spectre 进程的 `+mt`；如果用户已经在 `Spectre cmd` 中显式写了 `+mt=4` 之类的参数，则以用户填写为准。不同 Spectre 版本的多线程参数可能不同，必要时先用 `spectre -help all` 确认。

Spectre 建表默认采用保守并发策略：单个 Spectre 进程内部使用 `+mt`，并把 L/VDS/VSB 合并成大 batch，但默认不额外并发多个 Spectre batch，也默认不同时跑 NMOS/PMOS 两个器件。这是为了避免 WSL、VMware/VirtualBox 虚拟机、小型工作站在建表时 CPU 过载，导致 Virtuoso/X11/GUI 看起来卡死。Linux 服务器或大核心数机器可在 Advanced 设置里把 `Spectre workers` 或 `Device workers` 设为具体数字，例如 `2`；也可以用环境变量 `AMPSYS_SPECTRE_BATCH_WORKERS`、`AMPSYS_SPECTRE_DEVICE_WORKERS` 打开更激进并发。

Spectre 建表时，最终 `.pkl` 和 `*_data/` 仍写入用户选择的 `Cache dir`；临时 raw/psf 文件会优先写入 Linux 本地 scratch。如果 `Temp dir` 位于 WSL `/mnt/*`、VMware/VirtualBox shared folder、NFS/SMB 等慢盘，AmpSys 会自动把 transient raw 迁移到类似 `/tmp/ampsys_spectre_<uid>` 的本地目录。需要手动指定 scratch 时，可设置：

```bash
export AMPSYS_SPECTRE_SCRATCH=/tmp/ampsys_spectre_$USER
```
...
完整 Spectre LUT 建好后，建议先和已有 HSPICE LUT 做一次结构/大小对比：

```bash
py -3 <plugin-root>/cli/ampsys_runner.py compare-cache \
  --reference /path/to/hspice_cache \
  --candidate /path/to/spectre_cache \
  --output /path/to/compare_cache.json
```

重点看 `.pkl` 数量、`*_data/` 数量、NMOS/PMOS 文件大小比例。大小不要求逐字节一致，但应接近同一数量级；如果 Spectre cache 明显小很多，先不要进入优化，优先检查 `.dc` 参数解析和日志。
